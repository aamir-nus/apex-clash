...
- current late-route craft recipes now include `black_flash_talisman` and `domain_amplification_charm`
- dungeon chamber variants can now alter live sentinel pressure through content-selected curse-family modifiers
- boss chambers now also inherit route/chamber pressure modifiers from the selected dungeon variant
...
- the current authored routes remain contract-valid: `dungeon_miniboss` for `detention_center_dungeon`
...
- the current authored routes remain contract-valid: `merger_miniboss` for `merger_ossuary_dungeon`
- the current authored routes remain contract-valid: `veil_boss_scroll` for `barrier_shrine_boss_vault`
- the current authored routes remain contract-valid: `shatter_boss_scroll` for `detention_center_boss_vault`
- the current authored routes remain contract-valid: `night_boss_scroll` for `collapsed_cathedral_barrier_boss_vault`
- the current authored routes remain contract-valid: `merger_boss_core` for `merger_ossuary_boss_vault`
...
The browser flow check now also proves the longer profile loop: one consumable use, one required successful craft, optional second craft when materials allow, late-route Black Flash and domain craft payoffs, save-slot creation, snapshot resume, live-profile resume, snapshot return, and manual sync.
The browser flow check now also verifies that fully completed routes stay visibly cleared in the hub after extraction.
The browser flow check also covers the current onboarding/readability baseline: HUD objective surfacing, first-run completion persistence, and the guided five-route path remaining stable after scene-marker polish.
The next browser-hardening target is broader content pacing and richer authored route depth beyond the current five-route slice.
